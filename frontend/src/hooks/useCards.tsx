...
type CardContextType = {
  ...
  songsTicked: (id: string) => void;
  resetSelectedSongs: () => void;
};
...
export function CardProvider({ children }: CardProviderProps) {
  ...
  const songsTicked = useCallback((id: string) => {
    setRawSelection((prevRawSelection) => {
      const newSelectedSongs = prevRawSelection?.includes(id)
        ? prevRawSelection.filter((rawSelection) => rawSelection !== id)
        : [...prevRawSelection, id];
