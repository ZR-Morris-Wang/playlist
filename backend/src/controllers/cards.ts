...
export const getCards = async (
  ...
  res: Response<GetCardsResponse | { error: string }>,
) => {
  const prelist_id = req.query.input;
  if (req) {
    try {
      const card = await CardModel.find({ list_id: prelist_id });
      if (!card && card !== undefined) {
        return res.status(404).json({ error: "id is not valid" });
      }

      const formattedCards = card.map((card) => ({
